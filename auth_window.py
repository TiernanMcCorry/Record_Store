...
class AuthWindow:
    ...
    def create_owner_tab(self):
        ...
        form_frame.grid_rowconfigure(11, weight=1)
        ...
        guest_btn = tk.Button(form_frame,
                            ...
                            fg=COLORS['secondary'],
                            relief='flat',
                            command=lambda: self.on_login_success(is_owner=False, user={'username': 'guest', 'role': 'guest'}),
                            cursor='hand2')
    ...
    def create_signup_form(self, parent):
        """Create customer signup form with artist option"""
        parent.grid_rowconfigure(0, weight=1)
        parent.grid_columnconfigure(0, weight=1)
        ...
        self.is_artist_var = tk.BooleanVar()
        artist_check = tk.Checkbutton(scrollable_frame,
                                     text="I am an artist (request to perform)",
                                     variable=self.is_artist_var,
                                     font=FONTS['caption_small'],
                                     bg=COLORS['card_bg'],
                                     fg=COLORS['fg'],
        ...
        artist_check.grid(row=len(fields)*2 + 1, column=0, sticky="w", pady=(10, 5))
        
        # Artist details frame (hidden initially)
        self.artist_frame = tk.Frame(scrollable_frame, bg=COLORS['card_bg'])
        self.artist_frame.grid(row=len(fields)*2 + 2, column=0, sticky="ew", pady=(0, 10))
        self.artist_frame.grid_columnconfigure(0, weight=1)
        self.artist_frame.grid_remove()
        
        artist_fields = [
            ("Stage Name", "artist_stage_name"),
            ("Bio (optional)", "artist_bio"),
            ("Genre", "artist_genre"),
            ("Website (optional)", "artist_website"),
            ("Artist Phone (optional)", "artist_phone")
        ]
        
        self.artist_entries = {}
        for i, (label, name) in enumerate(artist_fields):
            tk.Label(self.artist_frame,
                    text=label,
                    font=FONTS['caption'],
                    bg=COLORS['card_bg'],
                    fg=COLORS['fg']).grid(row=i*2, column=0, sticky="w", pady=(5, 2))
            entry = tk.Entry(self.artist_frame,
                           font=FONTS['body'],
                           bg=COLORS['entry_bg'],
                           fg=COLORS['fg'],
                           relief='flat')
            entry.grid(row=i*2+1, column=0, sticky="ew", pady=(0, 5))
            self.artist_entries[name] = entry
        
        def toggle_artist_frame():
            if self.is_artist_var.get():
                self.artist_frame.grid()
            else:
                self.artist_frame.grid_remove()
        
        self.is_artist_var.trace('w', lambda *args: toggle_artist_frame())
        
        # Terms
        self.terms_var = tk.BooleanVar()
        terms_frame = tk.Frame(scrollable_frame, bg=COLORS['card_bg'])
        terms_frame.grid(row=len(fields)*2 + 3, column=0, sticky="ew", pady=15)
        ...
        self.signup_error = tk.Label(scrollable_frame,
                                   ...
                                   bg=COLORS['card_bg'],
                                   fg=COLORS['danger'])
        self.signup_error.grid(row=len(fields)*2 + 4, column=0, pady=(10, 0))
        ...
        signup_btn = tk.Button(scrollable_frame,
                             ...
                             padx=20,
                             pady=12)
        signup_btn.grid(row=len(fields)*2 + 5, column=0, sticky="ew", pady=(20, 30))
    ...
    def customer_signup(self):
        ...
        data = {
            ...
            'role': 'artist' if self.is_artist_var.get() else 'customer'
        }
        ...
        try:
            # Register customer
            customer_id = self.db.register_customer(data)
            
            # If artist, also create artist profile
            if data['role'] == 'artist':
                artist_data = {
                    'stage_name': self.artist_entries['artist_stage_name'].get().strip(),
                    'bio': self.artist_entries['artist_bio'].get().strip(),
                    'genre': self.artist_entries['artist_genre'].get().strip(),
                    'website': self.artist_entries['artist_website'].get().strip(),
                    'phone': self.artist_entries['artist_phone'].get().strip()
                }
                # If stage name is empty, use username
                if not artist_data['stage_name']:
                    artist_data['stage_name'] = data['username']
                self.db.register_artist(customer_id, artist_data)
    ...
    def owner_login(self):
        ...
        if username == self.owner_username and password == self.owner_password:
            # Create a user dict with owner role
            user = {'username': 'owner', 'role': 'owner', 'id': 0}
            self.on_login_success(is_owner=True, user=user)
        else:
